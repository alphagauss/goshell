...
func (s *ConfigService) Get(category, key string) (interface{}, error) {
	...
	if category == "terminal" {
		if key == "defaultType" {
			return s.config.Terminal.DefaultType, nil
		}
		if key == "autoSwitchClassic" {
			return s.config.Terminal.AutoSwitchClassic, nil
		}
		if key == "switchMode" {
			return s.config.Terminal.SwitchMode, nil
		}
		if key == "fontSize" {
			return s.config.Terminal.FontSize, nil
		}
		if key == "commandSendMode" {
			return s.config.Terminal.CommandSendMode, nil
		}
		if key == "codeHighlight" {
			return s.config.Terminal.CodeHighlight, nil
		}
	}

	if category == "ui" {
		if key == "autoTray" {
			return s.config.UI.AutoTray, nil
		}
		if key == "rememberPosition" {
			return s.config.UI.RememberPosition, nil
		}
		if key == "autoShowHome" {
			return s.config.UI.AutoShowHome, nil
		}
		if key == "theme" {
			return s.config.UI.Theme, nil
		}
	}

	if category == "cloud" {
		if key == "enabled" {
			return s.config.Cloud.Enabled, nil
		}
		if key == "serverUrl" {
			return s.config.Cloud.ServerURL, nil
		}
		if key == "token" {
			return s.config.Cloud.Token, nil
		}
		if key == "syncInterval" {
			return s.config.Cloud.SyncInterval, nil
		}
		if key == "autoSyncTo" {
			return s.config.Cloud.AutoSyncTo, nil
		}
		if key == "autoSyncFrom" {
			return s.config.Cloud.AutoSyncFrom, nil
		}
	}
	...
	if category == "shortcuts" {
		...
		if key == "switchTab" {
			return s.config.Shortcuts.SwitchTab, nil
		}
		if key == "saveGroup" {
			return s.config.Shortcuts.SaveGroup, nil
		}
		if key == "cloudUpload" {
			return s.config.Shortcuts.CloudUpload, nil
		}
		if key == "cloudDownload" {
			return s.config.Shortcuts.CloudDownload, nil
		}
	}

	if category == "advanced" {
		if key == "groupBehavior" {
			return s.config.Advanced.GroupBehavior, nil
		}
	}
...
func (s *ConfigService) Set(category, key string, value interface{}) error {
	...
	if category == "terminal" {
		if key == "defaultType" {
			if v, ok := value.(string); ok {
				s.config.Terminal.DefaultType = v
				return s.save()
			}
		...
		if key == "autoSwitchClassic" {
			if v, ok := value.(bool); ok {
				s.config.Terminal.AutoSwitchClassic = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
		if key == "switchMode" {
			if v, ok := value.(string); ok {
				s.config.Terminal.SwitchMode = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
		if key == "fontSize" {
			if v, ok := value.(float64); ok {
				s.config.Terminal.FontSize = int(v)
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
		if key == "commandSendMode" {
			if v, ok := value.(string); ok {
				s.config.Terminal.CommandSendMode = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
		if key == "codeHighlight" {
			if v, ok := value.(bool); ok {
				s.config.Terminal.CodeHighlight = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
	...
	if category == "ui" {
		...
		if key == "rememberPosition" {
			if v, ok := value.(bool); ok {
				s.config.UI.RememberPosition = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
	...
	if category == "cloud" {
		...
		if key == "syncInterval" {
			if v, ok := value.(float64); ok {
				s.config.Cloud.SyncInterval = int(v)
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
	...
	if category == "shortcuts" {
		...
		if key == "switchTab" {
			if v, ok := value.(bool); ok {
				s.config.Shortcuts.SwitchTab = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
		if key == "saveGroup" {
			if v, ok := value.(bool); ok {
				s.config.Shortcuts.SaveGroup = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
		if key == "cloudUpload" {
			if v, ok := value.(bool); ok {
				s.config.Shortcuts.CloudUpload = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
		if key == "cloudDownload" {
			if v, ok := value.(bool); ok {
				s.config.Shortcuts.CloudDownload = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
	...
	if category == "advanced" {
		if key == "groupBehavior" {
			if v, ok := value.(string); ok {
				s.config.Advanced.GroupBehavior = v
				return s.save()
			}
			return fmt.Errorf("无效的值类型")
		}
